...
void OLED_GPIO_Init(void)
{
    ...
    /* 将 SCL 和 SDA 引脚初始化为开漏模式 */
    GPIO_InitStructure.Pin = SCL_Pin | SDA_Pin;
    GPIO_InitStructure.Mode = GPIO_MODE_OUTPUT_OD;     // 开漏输出模式
    GPIO_InitStructure.Speed = GPIO_SPEED_FREQ_HIGH;   // 设置输出速度
    HAL_GPIO_Init(SCL_Port, &GPIO_InitStructure);
    HAL_GPIO_Init(SDA_Port, &GPIO_InitStructure);
		
    /* 释放 SCL 和 SDA 引脚 */
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_1, GPIO_PIN_SET); // 设置 SCL 为高电平
    HAL_GPIO_WritePin(GPIOA, GPIO_PIN_0, GPIO_PIN_SET); // 设置 SDA 为高电平
}
